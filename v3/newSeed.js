...
myDatabase = mongoose.connect("mongodb://localhost/game_campV2");
...
app.get("/seed", function (req, res) {
	...
	console.log("about to write new data on DATABASE V2");
...
//	res.send("you hit post route");
	var newGame = req.sanitize(req.body.name);
	var newImage = req.sanitize(req.body.image);
    var desc = req.sanitize(req.body.description);
    var story = req.sanitize(req.body.story);
	var price = req.sanitize(req.body.price);
	var owner = req.sanitize(req.body.owner);

	var newGame = {name: newGame, image: newImage, description: desc, story: story, price: price, owner: owner};
...
{
    console.log("camp server started - on DATABASE V2");
});
